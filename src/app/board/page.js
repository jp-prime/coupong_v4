...
export async function generateMetadata({ searchParams }) {
    ...
    if (!id && !slug) {
        return {
            title: '쿠퐁온라인 커뮤니티 | 베트남 호치민 라이프 소식',
    ...
    if (slug) {
        try {
            ...
            if (title) {
                const titleStr = `${title} | 쿠퐁온라인 커뮤니티`;
                const descStr = description ? String(description).substring(0, 150) : '쿠퐁온라인 커뮤니티 최신 게시글 내용';
                return {
                    title: titleStr,
                    description: descStr,
    ...
    try {
        ...
        if (post) {
            const titleStr = `${post.title} | 쿠퐁온라인 커뮤니티`;
            const descStr = `${post.content ? post.content.substring(0, 150) : '쿠퐁온라인 커뮤니티 최신 게시글 내용'}`;
